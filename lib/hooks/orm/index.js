...
// TODO: extrapolate to balderdashy/sails-stringfile
var logDeprecationNotice = require('../../deprecated').logDeprecationNotice;
var logUpgradeNotice = require('../../deprecated').logUpgradeNotice;
var LINKS = require('../../links');
var NOTICES = {
	upgrade: {
		'config.models.connection': 'For now, I\'ll pretend you set `config.models.connection`.',
		'config.connections': 'For now, I\'ll pretend you set `config.connections`.',
		'config.connections.*.adapter': 'For now, I\'ll pretend you set `config.connections["%s"].adapter`.'
	}
};
			...
			if (sails.config.adapters) {
				...
				if (sails.config.adapters['default']) {
					...
					sails.after('lifted', function () {
						...
						logDeprecationNotice(
							...
							LINKS.docs.migrationGuide.connections,
							sails.log.debug) &&
						logUpgradeNotice(NOTICES.upgrade['config.models.connection'], [], sails.log.debug);
				...
				sails.after('lifted', function () {
					...
					logDeprecationNotice(
						...
						LINKS.docs.migrationGuide.connections,
						sails.log.debug) &&
					logUpgradeNotice(NOTICES.upgrade['config.connections'], [], sails.log.debug);
				...
				_.each(sails.config.adapters, function (legacyAdapterConfig, connectionName) {
					...
					sails.after('lifted', function () {
						...
						logDeprecationNotice(
							...
							LINKS.docs.migrationGuide.connections,
							sails.log.debug) &&
						logUpgradeNotice(NOTICES.upgrade['config.connections.*.adapter'], [connectionName], sails.log.debug);
