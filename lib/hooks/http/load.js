...
module.exports = function (sails) {
    ...
    if (sails.config.mounts) {
      var mountsContainer = require('express')();
      sails.mountsContainer = mountsContainer;
      app.use(mountsContainer);

      // When Sails mounts an application, bind it to the mountsContainer
      sails.on('mounts:mount', function (appCfg) {
        util.each(util.flatten([appCfg.mount]), function(mt) {
          mountsContainer.use(appCfg.mountPath, mt);
        });
      });
		}
